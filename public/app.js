...
async function translateSection(section, language) {
    ...
    const languageNames = { mr: 'Marathi', hi: 'Hindi' };
    const targetLanguage = languageNames[language];

    let text = '';
    if (section === 'simplified') text = simplifiedOutput.dataset.originalText;
    else if (section === 'deepdive') text = deepDiveOutput.dataset.originalText;
    else if (section === 'flashcards') {
        const cards = JSON.parse(flashcardsOutput.dataset.originalCards);
    ...
    try {
        ...
        if (result.success) {
            if (section === 'simplified') {
                ...
                deepDiveOutput.innerHTML = result.translatedText.split('\n\n').map(p => `<p>${p}</p>`).join('');
            } else if (section === 'flashcards') {
                try {
                    const translatedCards = JSON.parse(result.translatedText);
                    flashcardsOutput.innerHTML = `
                        <div class="flashcards-grid">
