...
function displayResults(outputs, selectedOutputs) {
    ...
    if (selectedOutputs.visual && outputs.visual) {
        ...
        if (visualData.imageUrl) {
            ...
            const img = new Image();
            img.onload = () => {
                visualOutput.innerHTML = ''; // Clear loading
        ...
        } else if (visualData.type === 'error') {
            visualOutput.innerHTML = `<div class="error-box">
                <p>⚠️ Image Generation Failed</p>
                <p class="error-detail">${visualData.text}</p>
            </div>`;
        } else {
            // Fallback text/mermaid logic (preserved but likely unused with new image focus)
